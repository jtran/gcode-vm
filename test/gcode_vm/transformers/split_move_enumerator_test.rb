...
describe GcodeVm::SplitMoveEnumerator do
  ...
  it "indicates that it needs an evaluator injected" do
    GcodeVm::SplitMoveEnumerator.needs.must_equal :evaluator
  end
  ...
  it "doesn't split when equal to max distance" do
    ...
    enum.to_a.must_equal ['G1 X1.0 F1000']
  end
  ...
  it "ignores distance of non-distance axes" do
    ...
    enum.to_a.must_equal ['G1 X0.5 Y0.5 F1000', 'G1 X1.0 Y1.0 F1000']
    ...
    enum.to_a.must_equal ['G1 X1.0 Y0.5 F1000', 'G1 X2.0 Y1.0 F1000']
  end
  ...
  it "splits using current position of the evaluator in absolute mode" do
    ...
    enum.to_a.must_equal ['G1 X101.0 F1000', 'G1 X102.0 F1000']
  end
  ...
  it "splits based on max distance using multiple axes" do
    ...
    enum.to_a.must_equal ['G1 X0.333333 Y0.666667 E0.333333 F1000', 'G1 X0.666667 Y1.333333 E0.666667 F1000', 'G1 X1.0 Y2.0 E1.0 F1000']
    enum.source_enum = ['G1 X3.0 Y4.0 F1000'].each
    enum.to_a.must_equal ['G1 X0.6 Y0.8 F1000', 'G1 X1.2 Y1.6 F1000', 'G1 X1.8 Y2.4 F1000', 'G1 X2.4 Y3.2 F1000', 'G1 X3.0 Y4.0 F1000']
  end

  it "allows nil and other unrecognized strings to pass through" do
    enum = GcodeVm::SplitMoveEnumerator.new(axes: [:X, :Y, :Z, :E], max_distance: 1.0)
    enum.source_enum = ['foo', nil, ''].each
    enum.to_a.must_equal ['foo', nil, '']
  end
  ...
  it "splits and transforms" do
    ...
    enum.to_a.must_equal ['G1 X1.0 F1000 ; SOME COMMENT', 'G1 X2.0 F1000 ; SOME COMMENT']
  end
